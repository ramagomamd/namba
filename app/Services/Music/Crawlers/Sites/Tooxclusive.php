...
class Tooxclusive 
{
	public static function scan($client, $data)
	{
		...
		while ((is_null($results) || $page <= $data['pages']) && $done == false) {
			...
			$results[] = $crawler->filter('div.clear a.post-thumb')->each(function ($result) use ($client, $data) {
				...
				if ($mp3s->isNotEmpty()) {
					try{
					    $title = $crawler->filter('div.hentry-meta h1')->first()->text();
					} catch(Exception $e) { // I guess its InvalidArgumentException in this case
					    // Node list is empty
					    $title = null;
					}
					try{
					    $cover = $crawler->filter('div.hentry-content img')->first()->attr('src');
					} catch(Exception $e) { // I guess its InvalidArgumentException in this case
					    // Node list is empty
					    $cover = null;
					}
